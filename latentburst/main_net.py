...
class LatentBurst(nn.Module):
    def __init__(
        ...
        self.extract = FeatureExtractor(in_ch, filters, sub_levels)
    ...
    def debug_forward(self, x0, x1, batch_dt=0.5) -> Dict[str, List[torch.Tensor]]:
        ...
        backward_flow = backward_flow_pyramid #util.multiply_pyramid(backward_flow_pyramid, batch_dt)
        forward_flow = forward_flow_pyramid #util.multiply_pyramid(forward_flow_pyramid, 1 - batch_dt)
        ...
        # print(f'size of aligned_pyramid',aligned_pyramid[0].shape )
    ...
    def forward(self, x0, x1) -> torch.Tensor:
        return self.debug_forward(x0, x1)
