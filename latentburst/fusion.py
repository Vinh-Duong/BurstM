...
def get_channels_at_level(level, filters):
    ...
    flows = 2
    # print(f'get_channels_at_level', (sum(filters << i for i in range(level)) + channels + flows) * n_images)
...
class Fusion(nn.Module):
    ...
    def __init__(self, n_layers=4, specialized_layers=3, filters=64):
        ...
        self.up1 = nn.Sequential(nn.Conv2d(filters, filters*4, kernel_size=1, stride=1, padding=0, bias=False),
    ...
    def forward(self, pyramid: List[torch.Tensor]) -> torch.Tensor:
        ...
        for k, layers in enumerate(self.convs):
            ...
            # print(f'net shape 1',net.shape)
            net = F.interpolate(net, size=level_size, mode='nearest')
            b,c,h,w = net.shape
            # print(f'net shape 2',net.shape)
            if c % 4 == 0 and c % 3 == 0:
                net = net[:,1:c-1,:,:]
            # print(f'net shape 3',net.shape)
            
            net = layers[0](net)
            net = torch.cat([pyramid[i], net], dim=1)
            # print(f'net shape 4',net.shape)
            b,c,h,w = net.shape
            if c % 4 == 0:
                net = net[:,1:c-1,:,:]
            # print(f'net shape 5',net.shape)
